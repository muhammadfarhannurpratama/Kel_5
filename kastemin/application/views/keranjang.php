    ...
    <div class='container content' style="margin-top:9%">
      <div class="row">
        <div class="col-md-5">
            ...
            <?php } else{?>
            <h5>Keranjang Belanja Dengan Email <?php echo $sessionemail; ?></h5>
            <br>
            <?php } }?>
      ...
      <?php foreach ($cart as $key => $val) {
        ...
        $harga_satuan = $val['harga']/$val['jumlah'];

        $berat_jersey = $val['berat']*$val['jumlah'];

        $total_akhir = $total_akhir+$val['harga'];

        $total_qty = $total_qty+$val['jumlah'];

        $total_berat = $total_berat+$berat_jersey;

        $ppn = $total_akhir/100*10;

        $prev_array = $key - 1;

        $prev = isset($cart[$prev_array]) ? strtotime($cart[$prev_array]['tanggal']) : 0;
        ...
        <div class="row">
          ...
          <?php
                  ...
                  if($key==0 || $prev==strtotime($val['tanggal']))
                  {
                    //echo $val['tanggal'];
                  }
                  else
